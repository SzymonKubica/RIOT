...
/* Wrappers for storage functions needed because  they use a different helper calling convention. */
uint32_t f12r_vm_store_local(f12r_t *bpf, uint64_t *regs)
{

    return bpf_vm_store_local(bpf, regs[1], regs[2], regs[3], regs[4], regs[5]);
}

uint32_t f12r_vm_store_global(f12r_t *bpf, uint64_t *regs)
{
    return bpf_vm_store_global(bpf, regs[1], regs[2], regs[3], regs[4], regs[5]);
}

uint32_t f12r_vm_fetch_local(f12r_t *bpf, uint64_t *regs)
{
    return bpf_vm_fetch_local(bpf, regs[1], regs[2], regs[3], regs[4], regs[5]);
}

uint32_t f12r_vm_fetch_global(f12r_t *bpf, uint64_t *regs)
{
    return bpf_vm_fetch_global(bpf, regs[1], regs[2], regs[3], regs[4], regs[5]);
}
...
uint32_t f12r_vm_memcpy(f12r_t *f12r, uint64_t *regs)
{
    ...
    void *dest = (void *)(intptr_t)regs[1];
    const void *src = (const void *)(intptr_t)regs[2];
...
uint32_t f12r_vm_ztimer_periodic_wakeup(f12r_t *f12r, uint64_t *regs)
{
    ...
    // TODO: figure out why f12r uses registers starting at 0 whereas the
    // compiled code puts the arument into the register 1
    /* Old code:
...
uint32_t f12r_vm_printf(f12r_t *f12r, uint64_t *regs)
{
    ...
    LOG_DEBUG("f12r_vm_printf: %d, %d, %d, %d, %d \n", (intptr_t)regs[1],
              ...
              (intptr_t)regs[5]);
    return printf((const char *)(intptr_t)regs[1],
