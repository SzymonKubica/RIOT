...
/// @param[in]  location  SUIT ram storage location from where the bytecode is loaded.
uint32_t load_bytes_from_suit_storage(uint8_t *buff, char *location) {
  ...
  LOG_DEBUG("[SUIT storage loader]: setting suit storage active location: %s\n",
            location);
  ...
  LOG_DEBUG("[SUIT storage loader]: getting a pointer to the data stored in the SUIT "
            "location: %s.\n", location);
